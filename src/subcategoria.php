...
switch ($_GET["op"]){
	case '0':
		$rspta=$subcategoria->listar();
 		...
 		while ($reg = pg_fetch_assoc($rspta)){
			$data[]=array(
				"0"=>$reg['subcategorianombre'],
				"1"=>$reg['categorianombre'],
				"2"=>($reg['subcategoriacondicion'])?'<span class="badge badge-outline-success">Activo</span>':
					'<span class="badge badge-outline-danger">Desactivo</span>',
				"3"=>($reg['subcategoriacondicion'])?'<div class="flex gap-4 items-center">
													<a href="#" onclick="mostrar('.$reg['idsubcategoria'].')" class="hover:text-info" x-tooltip="Editar">
													...
													<a href="#" onclick="desactivar('.$reg['idsubcategoria'].')" class="hover:text-danger" x-tooltip="Eliminar">
					...
					<a href="#" onclick="activar('.$reg['idsubcategoria'].')" class="hover:text-success" x-tooltip="Eliminar">
						<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5">
	...
	case '1':
		if (empty($idsubcategoria)){
			$rspta=$subcategoria->insertar($nombre, $categoria);
			echo $rspta ? "1:La Subcategoría fué registrada" : "0:La Subcategoría no fué registrada";
		}
		else {
			$rspta=$subcategoria->editar($idsubcategoria, $nombre, $categoria);
			echo $rspta ? "1:La Subcategoría fué actualizada" : "0:La Subcategoría no fué actualizada";
		}
	break;

	case '2':
		$rspta=$subcategoria->desactivar($idsubcategoria);
 		echo $rspta ? "1:La Subcategoría fué Desactivada" : "0:La Subcategoría no fué Desactivada";
	break;

	case '3':
		$rspta=$subcategoria->activar($idsubcategoria);
 		echo $rspta ? "1:La Subcategoría fué Activada" : "0:La Subcategoría no fué Activada";
	break;
	...
	case '5':
		...
		{
			if($contador==0){
				echo '<option value=' . $reg['idsubcategoria'] . ' selected>' . $reg['subcategorianombre'] . '</option>';
			}
			else{
				echo '<option value=' . $reg['idsubcategoria'] . '>' . $reg['subcategorianombre'] . '</option>';
			}
		}
	break;
}
