...
switch ($_GET["op"]){
	case '0':
		$rspta=$categoria->listar();
 		...
 		while ($reg = pg_fetch_assoc($rspta)){
			$data[]=array(
				"0"=>$reg['categorianombre'],
				"1"=>($reg['categoriacondicion'])?'<span class="badge badge-outline-success">Activo</span>':
					'<span class="badge badge-outline-danger">Desactivo</span>',
				"2"=>($reg['categoriacondicion'])?'<div class="flex gap-4 items-center">
													<a href="#" onclick="mostrar('.$reg['idcategoria'].')" class="hover:text-info" x-tooltip="Editar">
													...
													<a href="#" onclick="desactivar('.$reg['idcategoria'].')" class="hover:text-danger" x-tooltip="Eliminar">
					...
					<a href="#" onclick="activar('.$reg['idcategoria'].')" class="hover:text-success" x-tooltip="Eliminar">
						<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5">
	...
	case '1':
		if (empty($idcategoria)){
			...
			echo $rspta ? "1:La Categoría fué registrada" : "0:La Categoría no fué registrada";
		}
		else {
			$rspta=$categoria->editar($idcategoria,$nombre);
			echo $rspta ? "1:La Categoría fué actualizada" : "0:La Categoría no fué actualizada";
		}
	break;

	case '2':
		$rspta=$categoria->desactivar($idcategoria);
 		echo $rspta ? "1:La Categoría fué Desactivada" : "0:La Categoría no fué Desactivada";
	break;

	case '3':
		$rspta=$categoria->activar($idcategoria);
 		echo $rspta ? "1:La Categoría fué Activada" : "0:La Categoría no fué Activada";
	break;
	...
	case '5':
		...
		{
			if($contador==0){
				echo '<option value=' . $reg['idcategoria'] . ' selected>' . $reg['categorianombre'] . '</option>';
			}
			else{
